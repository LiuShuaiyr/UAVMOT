...
def _neg_loss(pred, gt):
    ...
    # attention_wights = torch.Tensor(neg_weights.size()).fill_(1).cuda()
    # for i in range(10):
    #     if pos_inds[:,i,:,:].float().sum()==0:
    #         rate=1
    #     else:
    #         rate = pos_inds.float().sum() / pos_inds[:, i, :, :].float().sum()
    #         rate = torch.clamp(rate,0,20)
    #     attention_wights[:, i, :, :] = rate
    loss = 0
