...
class MOT(nn.Module):
    ...
    def forward(self, pre_x,x):
        ...
        # pre_out_corr_id = self.corr(pre_output['id'], pre_output['id'])  # False
        # pre_out_corr_id = self.flow_conv(pre_out_corr_id)
        # pre_output['id'] = pre_output['id'] + pre_out_corr_id
        ...
        # Merge streams
        # out_corr = self.corr(pre_y[-1],y[-1])  # False
        # out_corr = self.corr_activation(out_corr)
        # out_conv_redir = self.conv_redir(y[-1])
        # out_corr = torch.cat((out_conv_redir, out_corr), 1)
        # Y_net = self.flow_conv(out_corr)
        # guidance=torch.relu(Y_net)*pre_output['id']

        # Redirect top input stream and concatenate
        # out_conv_redir = self.conv_redir(pre_y[-1])

        # in_conv3_1 = torch.cat((out_conv_redir, out_corr), 1)
        # in_conv3_1 = self.corr_activation(out_corr)
        # y[-1] = y[-1] * torch.sigmoid(pre_y[-1])
        # 网络前向计算返回一个dict输出
        # Y_net = self.flow_conv(out_corr)
        # Y_net = F.relu(in_conv3_1)
        ...
        # scores, inds, classes, ys, xs, cls_inds_masks = _topk(heatmap=heatmap, K=200, num_classes=10)
        batch, C, H, W = heatmap.size()
        K = 100
        topk_scores, topk_inds = torch.topk(heatmap.view(batch, C, -1), K)
        topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)
        # topk_clses = (topk_ind / 200).int()
        ...
        #
        # # 方案一 exp_5
        # # change_feature = corr.unsqueeze(1)*tt.unsqueeze(3).unsqueeze(4)
        # # change_feature = self.liner(change_feature.permute(0,1,3,4,2)).squeeze(4)
        # # output['id'] = change_feature + output['id']
        # # 方案二 exp_6
        # # change_feature = self.liner1(corr.permute(0, 2, 3, 1)).squeeze(3)
        # # mask = change_feature>0.3
        # # pre_feature = self.liner2(tt)
        # # change_feature = change_feature.unsqueeze(1) * pre_feature.unsqueeze(3) * mask.unsqueeze(1).float()
        ...
        # # change_feature = corr.unsqueeze(4).permute(0,4,2,3,1) * pre_feature.unsqueeze(2).unsqueeze(3) * mask.unsqueeze(4).permute(0,4,2,3,1).float()
        ...
        # # 方案 exp_10
        # corr = self.relu(corr)
        # corr_max_index = torch.max(corr,1)[1].view(batch,H*W)
        # curret_id_feature = []
        # for i in range(batch):
        #     curret_feature = torch.index_select(tt[i, :, :], 1, corr_max_index[i, :])
        #     curret_id_feature.append(curret_feature.view(1,128,H,W))
        # change_feature= torch.cat(curret_id_feature, 0)
        #
        # change_feature = self.relu(change_feature)
        #
        # id_feature = torch.cat([change_feature, output['id']], dim=1)
        # output['id'] = self.conv_id_v1(id_feature)
        for head in self.heads:
            if head=='id':
                continue
