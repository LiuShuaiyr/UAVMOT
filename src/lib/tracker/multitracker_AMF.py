...
from lib.utils.rigd_transform import rigid_transform_2D
from gen_dataset_visdrone import cls2id, id2cls  # visdrone
from lib.tracker.matching import local_relation_fuse_motion
...
class MCTrack(MCBaseTrack):
    ...
    def AMF_re_activate(self, new_track, frame_id, new_id=False):
        # self.mean, self.covariance = self.kalman_filter.update(self.mean,
        #                                                        self.covariance,
        #                                                        self.tlwh_to_xyah(new_track.tlwh))
        # self.mean, self.covariance=None,None
        ...
        self.is_activated = True
        self.frame_id = frame_id
        self._tlwh[0:4] = new_track.tlwh[0:4]
        self.mean, self.covariance = self.kalman_filter.initiate(self.tlwh_to_xyah(self._tlwh))
    ...
    def update(self, new_track, frame_id, update_feature=True):
        ...
        self.stat7e = TrackState.Tracked  # set flag 'tracked'
    ...
    def AMF_update(self, new_track, frame_id, update_feature=True):
        """
        Update a matched track
        :type new_track: Track
        :type frame_id: int
...
class MCJDETracker_AMF(object):
    ...
    def update_detection(self, pre_im_blob,im_blob, img_0):
        """
        更新视频序列或图片序列的检测结果
        :rtype: dict
        :param im_blob:
    ...
    def update_tracking(self, pre_im_blob,im_blob, img_0):
        ...
        for cls_id in range(self.opt.num_classes):  # cls_id从0开始
            ...
            AMF_pool_dict =track_pool_dict[cls_id]

            # Predict the current location with KF
            Track.multi_predict(track_pool_dict[cls_id])
            dists = matching.embedding_distance(track_pool_dict[cls_id], cls_detects)
            feature_dists = dists.copy()
            feature_matches, feature_track, feature_detection = matching.linear_assignment(dists, thresh=0.7)
            ...
            if type(u_detection)==tuple:
                flag=0
            else:
                if cls_detects ==[]:
                    flag=0
                else:
                    if not u_detection.any():
                        flag = 0
                    else:
                        if u_detection.shape[0] / (len(cls_detects) + 0.1) < 0.6:
                            flag = 0
                        else:
                            if reference_detect==[]:
                                flag=0
            ...
            if flag==0:
                ...
                ### 选择二 使用设计的local relation filter进行前后帧关联
                if not matches.any():
                    matches, u_track, u_detection = feature_matches, feature_track, feature_detection
                else:
                    feature_dists = local_relation_fuse_motion(feature_dists,tracks=track_pool_dict[cls_id],detections=cls_detects,only_position=False,lambda_=0.9)
                    matches, u_track, u_detection = matching.linear_assignment(feature_dists, thresh=0.8)
                ...
                for i_tracked, i_det in matches:
                    ...
                    det = cls_detects[i_det]
                    if track.state == TrackState.Tracked:
                        track.AMF_update(cls_detects[i_det], self.frame_id)
                        activated_tracks_dict[cls_id].append(track)  # for multi-class
                    else:
                        track.AMF_re_activate(det, self.frame_id, new_id=False)
                        refined_tracks_dict[cls_id].append(track)
                ''' Step 3: Second association, with IOU'''


                for it in u_track:
                    track = AMF_pool_dict[it]
                    if not track.state == TrackState.Lost:
                        track.mark_lost()
                        lost_tracks_dict[cls_id].append(track)
